  ...
  return (
    <div className="w-[340px] text-[11px] leading-tight text-black">
      ...
      <div className="mb-2 flex items-center gap-1.5">
        <Image src="/batac-seal.jpg" alt="" width={32} height={32} className="shrink-0" />
        <h1 className="text-sm font-bold tracking-wide">DAILY TIME RECORD</h1>
      </div>

      <div className="mb-0.5 border-b border-black pb-0.5 text-center font-medium">{employeeName}</div>
      <p className="mb-1 text-center text-[10px] italic">(NAME)</p>

      <div className="mb-0.5 border-b border-black pb-0.5 text-center">{officeAssignment}</div>
      <p className="mb-2 text-center text-[10px] italic">(DEPARTMENT)</p>
      ...
      <table className="w-full border-collapse text-[10px] leading-none">
        <thead>
          <tr className="text-brand-gold">
            <th rowSpan={2} className="border border-black py-px px-0.5 align-middle">
              Day
            </th>
            <th colSpan={2} className="border border-black py-px px-0.5">
              A.M.
            </th>
            <th colSpan={2} className="border border-black py-px px-0.5">
              P.M.
          ...
          <tr className="text-primary">
            <th className="border border-black py-px px-0.5 font-normal">Arrival</th>
            <th className="border border-black py-px px-0.5 font-normal">Departure</th>
            <th className="border border-black py-px px-0.5 font-normal">Arrival</th>
            <th className="border border-black py-px px-0.5 font-normal">Departure</th>
            <th className="border border-black py-px px-0.5 font-normal">Hours</th>
            <th className="border border-black py-px px-0.5 font-normal">Minute</th>
          </tr>
        </thead>
        <tbody>
          {dates.map((date) => {
            ...
            return (
              <tr key={iso}>
                <td className="border border-black py-px px-0.5 text-center text-primary">{date.getUTCDate()}</td>
                {isSpecial ? (
                  <td colSpan={4} className="border border-black py-px px-0.5 text-center italic text-gray-600">
                ...
                <td className="border border-black py-px px-0.5 text-center">{day && day.lateMinutes > 0 ? day.lateMinutes % 60 : ""}</td>
              </tr>
            );
          })}
          <tr>
            <td colSpan={5} className="border border-black py-px px-0.5 text-right font-bold">
              TOTAL:
            </td>
            <td className="border border-black py-px px-0.5 text-center font-bold">{totalHours || ""}</td>
            <td className="border border-black py-px px-0.5 text-center font-bold">{totalMinutes || ""}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="mt-3 mb-0.5 border-b border-black" />
      <p className="text-[10px] italic">VERIFIED as to prescribed office hours</p>

      <div className="mt-3 mb-0.5 w-full border-b border-black" />
