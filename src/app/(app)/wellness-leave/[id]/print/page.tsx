...
export default async function WellnessLeavePrintPage({ params }: PageProps<"/wellness-leave/[id]/print">) {
  ...
  const dailyRate = rate ? getDailyRate(rate.monthlyAmount) : null;
      ...
      <div className="mx-auto max-w-3xl bg-white p-8 text-[13px] text-black print:m-0 print:max-w-none print:p-0">
        ...
        <div className="border border-black">
          ...
          <div className="flex border-b border-black">
            <Cell className="w-1/2 border-r border-black" label="1. OFFICE/DEPARTMENT" value={request.employee.officeAssignment} />
            <Cell className="w-1/2" label="2. NAME (Last, First, Middle)" value={request.employee.name} />
          </div>

          {/* 3-5: Date filed / Position / Salary */}
          <div className="flex border-b border-black">
            <Cell className="w-1/3 border-r border-black" label="3. DATE OF FILING" value={formatISODate(request.createdAt)} />
            <Cell className="w-1/3 border-r border-black" label="4. POSITION" value={request.employee.positionTitle} />
            <Cell
              className="w-1/3"
              label="5. SALARY"
              value={rate ? `${peso(rate.monthlyAmount)}/mo. (${peso(dailyRate!)}/day)` : "—"}
            />
          </div>
          ...
          <div className="flex border-b border-black">
            <div className="w-1/2 border-r border-black p-2">
              ...
              <p>☑ Wellness Leave — {semesterLabel(semester)}</p>
            ...
            <div className="w-1/2 p-2">
              ...
              <p className="mt-2">Reason/Notes: {request.notes || "—"}</p>
            </div>
          </div>
          ...
          <div className="flex border-b border-black">
            <div className="w-1/2 border-r border-black p-2">
              ...
              <p className="mb-2">{request.daysCount} day(s)</p>
          ...
          <div className="flex border-b border-black">
              ...
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="border border-black p-1 text-center">{semester === 1 ? request.daysCount : "-"}</td>
                    <td className="border border-black p-1 text-center">{semester === 2 ? request.daysCount : "-"}</td>
                  </tr>
            ...
            <div className="w-1/2 p-2">
              ...
              <p>{request.status === "APPROVED" ? "☑" : "☐"} For approval</p>
              <p>
                {request.status === "REJECTED" ? "☑" : "☐"} For disapproval due to
                {request.status === "REJECTED" ? " " : " ______________________"}
              </p>
          ...
          <div className="flex">
            <div className="w-1/2 border-r border-black p-2">
              ...
              <p>{request.status === "APPROVED" ? request.daysCount : "____"} days with pay</p>
              ...
              {request.approvedBy && request.approvedAt && (
                <>
                  ...
                  <p className="text-center text-xs">
                    ...
                    <br />
                    ({request.status === "APPROVED" ? "Approving Officer" : "Resolving Officer"}, resolved{" "}
                    {formatISODate(request.approvedAt)})
                  </p>
                </>
              )}
            </div>
...
function BalanceRow({
  ...
  label: string;
  sem1: { allotted: number; used: number } | null;
  sem2: { allotted: number; used: number } | null;
  field: "allotted" | "remaining";
}) {
  const value = (b: { allotted: number; used: number } | null) => {
    if (!b) return "—";
